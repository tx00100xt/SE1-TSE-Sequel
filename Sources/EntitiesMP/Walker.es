...
//      autocall CEnemyBase::LockOnEnemy() EReturn;
	...
	if (m_EwcChar==WLC_CANNON) {
      m_iLoopCounter = 4;
      while(m_iLoopCounter>0) {
         ANGLE3D _ANGLE3D_CANNON   = ANGLE3D(0.0f, 3.5f, 0.0f);
        if (m_iLoopCounter%2) {
          FLOAT3D _FIRE_LEFT_CANNON = (FLOAT3D(-2.5f, 5.0f, -1.0f))*m_fSize;
          StartModelAnim(WALKER_ANIM_FIRELEFT, AOF_LOOPING);
          ShootCannonball(_FIRE_LEFT_CANNON, _ANGLE3D_CANNON);

        } else {
          FLOAT3D _FIRE_RIGHT_CANNON = (FLOAT3D(+2.5f, 5.0f, -1.0f))*m_fSize;
          StartModelAnim(WALKER_ANIM_FIRERIGHT, AOF_LOOPING);
          ShootCannonball(_FIRE_RIGHT_CANNON, _ANGLE3D_CANNON);
 ...
 ************************************************************/
  Death(EVoid) : CEnemyBase::Death {
    ...
    if (m_EwcChar==WLC_CANNON) {
      if (IRnd()&1) {
        FLOAT3D _FIRE_DEATH_RIGHT = (FLOAT3D(3.75f, 4.2f, -2.5f))*m_fSize;
        FireDeathCannon(_FIRE_DEATH_RIGHT);
      } else {
        FLOAT3D _FIRE_DEATH_LEFT = (FLOAT3D( 0.0f, 7.0f, -2.0f))*m_fSize;
        FireDeathCannon(_FIRE_DEATH_LEFT);
      }
    ...
    if (m_EwcChar==WLC_MG) {
      if (IRnd()&1) {
        FLOAT3D _FIRE_DEATH_RIGHT = (FLOAT3D(3.75f, 4.2f, -2.5f))*m_fSize;
        FireDeathBullet(_FIRE_DEATH_RIGHT);
      } else {
        FLOAT3D _FIRE_DEATH_LEFT = (FLOAT3D( 0.0f, 7.0f, -2.0f))*m_fSize;
        FireDeathBullet(_FIRE_DEATH_LEFT);
      }
      PlaySound(m_soFire2, SOUND_GUNNER_FIRE_BULLET, SOF_3D);
    }
    autowait(0.25f);
