...
class CEnemyLauncher: CRationalEntity {
name      "Enemy Launcher";
thumbnail "Thumbnails\\EnemyLauncher.tbn";
...
properties:
  ...
  3 CTString m_strName          "Name" 'N' = "Enemy Launcher",
...
components:
  ...
  2 texture TEXTURE_ENEMYSPAWNER   "ModelsMP\\Editor\\EnemyLauncher.tex",
...
functions:
  ...
  {
    if( slPropertyOffset == offsetof(CEnemyLauncher, m_penTarget))
    {
      return CheckTemplateValid(penTarget);
    }
    else if( slPropertyOffset == offsetof(CEnemyLauncher, m_penPatrol))
    {
      return (penTarget!=NULL && IsDerivedFromClass(penTarget, "Enemy Marker"));
    }
    else if( slPropertyOffset == offsetof(CEnemyLauncher, m_penSeriousTarget))
    {
      return CheckTemplateValid(penTarget);
    }   
    else if( slPropertyOffset == offsetof(CEnemyLauncher, m_penEasyTarget))
    {
      return CheckTemplateValid(penTarget);
    }   
    else if( slPropertyOffset == offsetof(CEnemyLauncher, m_penTacticsHolder))
    {
  ...
  void SpawnEntity(BOOL bCopy) {
    ...
    if (CheckTemplateValid(m_penTarget)) {
      ...
      if (bCopy) {
       ...
       // set moving
       ((CEnemyBase&)*pen).LaunchAsFreeProjectile(FLOAT3D(0, m_ctVSpeed, -m_ctHSpeed), 0);
        if (m_penPatrol!=NULL) {
  ...
  {
    // initial
    SLONG slUsedMemory = sizeof(CEnemyLauncher) - sizeof(CRationalEntity) + CRationalEntity::GetUsedMemory();
    // add some more
