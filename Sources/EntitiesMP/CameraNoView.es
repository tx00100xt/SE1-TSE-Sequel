...
class CCameraNoView : CMovableModelEntity
{
name      "CameraNoView";
...
components:
  ...
  2 texture TEXTURE_CAMERA   "Models\\Editor\\CameraNoView.tex"
    ...
    if( tmCurrent > (m_tmAtMarker+m_tmDelta - _pTimer->TickQuantum*3/2)) 
    {
      ...
      FOREVER {
        ...
        if (pcmNp0->m_penTrigger!=NULL) {
          // trigger it
          SendToTarget(pcmNp0->m_penTrigger, EET_TRIGGER);
        }
  ...
  PlayAutoRotatingCamera()
  {
    ...
    wait() {
      on( EStop) : {
        ...
        {
          SendToTarget(m_penAutoCameraEndTarget, m_eetAutoCameraEndEvent);
        }
        return;
      }
      otherwise() : {
